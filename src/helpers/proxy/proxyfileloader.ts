...
export async function loadProxyFile(config: FileConfig) {
...
async function readProxyFile(file: string) {
  console.debug(`read proxy file : ${file}`);
  const lines = (await readFile(file as any)).toString().split('\n');
  const proxyList = new ProxyList();

  proxyList.setProxies(
    lines
...
function filterProxies(line: string) {
